...
if __name__ == "__main__":
    ...
    # Vectorize test data
    test_data_vectorized = convert_to_vector_representation(test, word2index)
    ...
    with open('results/test.out', 'w') as f:
        for input_vector, gold_label in test_data_vectorized:
            predicted_vector = model(input_vector)
            predicted_label = torch.argmax(predicted_vector)
            # Write prediction to file (add 1 to convert back to 1-5 star rating)
            f.write(f"{int(predicted_label) + 1}\n")
    
    print("Testing completed. Predictions written to results/test.out")